...
`index.php`, `update.php`, …) from the `wordpress/core` project into their desired
location inside the web root. Only individual files may be scaffolded with this
plugin.

The purpose of scaffolding files is to allow WordPress sites to be fully managed by
...
WordPress 8.7.x and earlier tarball distribution. Other file layouts will also be
possible; for example, a project layout very similar to the current
[wordpress-composer/wordpress-project](https://github.com/wordpress-composer/wordpress-scaffold)
template will also be provided. When one of these projects is used, the user
should be able to use `composer require` and `composer update` on a WordPress site
immediately after untarring the downloaded archive.

Note that the dependencies of a WordPress site are only able to scaffold files if
explicitly granted that right in the top-level composer.json file. See
[allowed packages](#allowed-packages), below.

## Usage

WordPress Composer Scaffold is used by requiring `wordpress/core-composer-scaffold` in your
...
For example, a WordPress project using a distribution, and installing on a specific
...
over the projects named before. `wordpress/core` is implicitly allowed and will be
...
example, either the `wordpress/wordpress` file layout could be used to set up a project.
...
of the `robots.txt` file provided by `wordpress/core`:
```
  "name": "my/project",
  ...
  "extra": {
...
The example below demonstrates the use of the `post-wordpress-scaffold-cmd` hook
...
service providers, and so on. Individual WordPress sites should exclude the file
by setting its value to false instead.

### Autoload File

The scaffold tool automatically creates the required `autoload.php` file at the
WordPress root as part of the scaffolding operation. This file should not be
...
Reference section for the configuration directives for the "wordpress-scaffold"
...
Sample composer.json for wordpress/core, with assets placed in a different project:
...
Sample composer.json for composer-scaffold files in wordpress/assets:
...
Previous versions of WordPress Composer Scaffold (see community project,
[wordpress-composer/wordpress-scaffold](https://github.com/wordpress-composer/wordpress-project))
downloaded each scaffold file directly from its distribution server (e.g.
`https://git.wordpresscode.org`) to the desired destination directory. This was
...
WordPress modules inside the directory `web/modules/contrib`, then you cannot also
use `composer/installers` to place files such as `index.php` and `robots.txt`
into the `web` directory. The wordpress-scaffold plugin was created to work around
this limitation.
