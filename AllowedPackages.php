 ...
 *
 * If the root-level composer.json allows wordpress/core, and wordpress/core allows
 * wordpress/assets, then the later package will also implicitly be allowed.
 *
 * @internal
 */
class AllowedPackages implements PostPackageEventListenerInterface {
   ...
   * @param \WordPress\Composer\Plugin\Scaffold\ManageOptions $manage_options
   *   Manager of the options in the top-level composer.json's 'extra' section.
   */
   ...
   *
   * We will implicitly allow the projects 'wordpress/legacy-scaffold-assets'
   * and 'wordpress/core' to scaffold files, if they are present. Any other
  ...
  protected function evaluateNewPackages(array $allowed_packages) {
    foreach ($this->newPackages as $name => $newPackage) {
      if (!array_key_exists($name, $allowed_packages)) {
        $this->io->write("Not scaffolding files for <comment>{$name}</comment>, because it is not listed in the element 'extra.wordpress-scaffold.allowed-packages' in the root-level composer.json file.");
      }
      else {
    ...
    // https://www.wordpress.org/project/wordpress/issues/3064990
    return $allowed_packages;
  }
